...
minisat_Var getVar(int index, int colour)
{
 ...
 int indexColourVar = 0.5 * (index + colour) * (index + colour + 1) + colour;
 return indexColourVar; 
...
void addColouringConstraint(int n)
{
 ...
 //TODO Write your code here
 minisat_Var colourArray[colours];
 ...
 {
   colourArray[colour] = getVar(n, colour);
 } // For
 
 atLeastOneOfThese(colourArray, colours);
...
int checkColouring(int* colouring)
{
  ...
  for(int i = 1; i < graph->MaxSize;i++) {
    List* ptr = graph->table[i].outlist;
    while (ptr != NULL) {
      if (colouring[i] == colouring[ptr->index])
        return 0;
