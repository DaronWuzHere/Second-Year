...
struct sigaction darons_sigstruct	// The data structurre I created
...
{
  darons_sigstruct.sa_handler = &broken;	// Pointer to the function 
						// broken
  sigemptyset(&darons_sigstruct.sa_mask);	// This is my call that defines
						// the empty
  daron_sigstruct.sa_flags	=0;		// No flags

  sigaction(SIGKILL, &darons_sigstruct.sa_handler, NULL);	//This is link
								// to the signal
...
void broken(int signum)	// The argument indicates which signal was activated
{
  ...
  switch (signum)	// signum values are defined in "signal.h"
  {
    ...
    // This is the signal I added to kill the process
    // To check if it is killed, I used & to keep the process running and 
    // checked the running processes. Then I cancelled the running program 
    // using kill
    case SIGKILL: printf("You killed and threrefore are a murderer.\n");break;
