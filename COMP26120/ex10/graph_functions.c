...
void insert_graph_node (Graph *mygraph, int n, char *name) {
  ...
  mygraph->table[n].nodeName = (char*)malloc(sizeof(char)*strlen(nodeName) + 1);
  strcpy(mygraph->table[n].nodeName, nodeName);  
  mygraph->outdegree = 0;
  mygraph->outlist = NULL;

} // Insert Graph Node

void insert_graph_link (Graph *mygraph, int source, int target) {

  // If the oultist is empty, create one
  if(mygraph->table[source].outlist == NULL) { 
    ...
    // If there is one, just insert it after the head
    List *head = malloc(sizeof(List));
    head->index = target;
    head->next = mygraph->table[source].outlist;
    mygraph->table[source].outlist->next = NULL;
  } // Else
