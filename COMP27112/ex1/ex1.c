...
void calculate_lookpoint(void) { /* Given an eyepoint and latitude and 
longitude angles, will compute a look point one unit away */
  ...
  // Variable to calculate the mouse lateral lookpoint
...
void keyboard(unsigned char key, int x, int y) {
  switch (key) {
    case 27:  /* Escape key */
      ...
      break;
    case 44:
      eyex = eyex + sin(DEG_TO_RAD * (lon + mlon + 90)) * RUN_SPEED;
      eyez = eyez + cos(DEG_TO_RAD * (lon + mlon + 90)) * RUN_SPEED; 
      break;
    case 46: 
      eyex = eyex + sin(DEG_TO_RAD * (lon + mlon - 90)) * RUN_SPEED;
      eyez = eyez + cos(DEG_TO_RAD * (lon + mlon - 90)) * RUN_SPEED; 
...
void cursor_keys(int key, int x, int y) {
  switch (key) {
    ...
    case GLUT_KEY_HOME:
      ...
      break;
    case GLUT_KEY_UP:
      eyex = eyex + sin(DEG_TO_RAD * (lon + mlon)) * RUN_SPEED;
      eyez = eyez + cos(DEG_TO_RAD * (lon + mlon)) * RUN_SPEED;
      break; 
    case GLUT_KEY_DOWN:
      eyex = eyex - sin(DEG_TO_RAD * (lon + mlon)) * RUN_SPEED;
      eyez = eyez - cos(DEG_TO_RAD * (lon + mlon)) * RUN_SPEED;
