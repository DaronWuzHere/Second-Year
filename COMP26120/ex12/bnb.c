...
void branch_and_bound(int *final_sol)
{
  ...
  while (QueueSize != 0 || solution.bound < current_best)
  {
    ...
    copy_array(solution.solution_vec, child1.solution_vec); 
    copy_array(solution.solution_vec, child2.solution_vec);
    
    child1.fixed = solution.fixed + 1;
    child2.fixed = solution.fixed + 1;

    child1.solution_vec[child1.fixed] = 0;
    child2.solution_vec[child2.fixed] = 1;
    ...
    if(child2.val != -1)
    {
      if(child2.val > current_best)
      {
        ...
        copy_array(child2.solution_vec, final_sol);
      }
      insert(child2);
    }
